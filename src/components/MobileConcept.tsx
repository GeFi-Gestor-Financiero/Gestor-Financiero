...
function HomeScreen({ navigate, userName, summary, transactions, accounts, loans, investmentBreakdown, settings, onSaveSettings, onAddTransaction, onDeleteTransaction, onEditTransaction, onAddLoan, onRepayPerson, onWithdrawInvestment, onClearInvestmentBalance }: { navigate: (screen: Screen) => void } & MobileConceptProps) {
  ...
  return <main className="mc-screen">
    ...
    <section className="mc-section mc-home-loans"><div className="mc-section-title"><div><h3>Préstamos realizados</h3><small>Total pendiente: {settings.hideBalances?'••••':money(debtTotal,settings.monedaBase)}</small></div><button onClick={()=>navigate('loan')}>Nuevo préstamo</button></div>{loanGroups.length?<div className="mc-loan-list">{loanGroups.map(group=><button key={group.person.toLowerCase()} onClick={()=>setLoanDetailsPerson(group.person)}><span><strong>{group.person}</strong><small>Tocá el monto para ver cada préstamo</small></span><b>{settings.hideBalances?'••••':money(group.total,settings.monedaBase)}</b></button>)}</div>:<div className="mc-empty-state"><HandCoins size={19}/><span>Todavía no registraste préstamos.</span></div>}</section>
