...
type MobileConceptProps = {
  ...
  accounts: Account[];
  loans: Loan[];
  investmentBreakdown: Array<{ name: string; value: number }>;
...
function HomeScreen({ navigate, userName, summary, transactions, accounts, loans, investmentBreakdown, settings, onSaveSettings, onAddTransaction, onDeleteTransaction, onEditTransaction, onAddLoan, onRepayPerson, onWithdrawInvestment }: { navigate: (screen: Screen) => void } & MobileConceptProps) {
  ...
  const selectedInvestment=investmentBreakdown.find(item=>item.name===investmentPlatform)?.value||0;
