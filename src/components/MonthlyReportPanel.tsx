...
export default function MonthlyReportPanel({ income, expense, investment, categories, transactions, month, year, currency, language = 'es', hidden = false }: Props) {
  ...
  return <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
    ...
    <div className="p-4">
      ...
      <div className="mt-5 border-t border-slate-100 pt-4 dark:border-slate-800"><div className="mb-3 flex items-center justify-between gap-3"><p className="text-[10px] font-bold uppercase tracking-wide text-slate-400">Gastos por categoría</p><span className="text-[10px] text-slate-400">{hidden ? '••••' : money(expense)}</span></div>{top.length ? <div className="space-y-3">{top.map(([name, value], index) => { const percentage = Math.round(value / categoryTotal * 100); const colors = ['bg-rose-500','bg-orange-500','bg-blue-500','bg-cyan-500','bg-violet-500']; return <div key={name}><div className="mb-1 flex items-center justify-between gap-3 text-xs"><span className="min-w-0 truncate font-medium">{name}</span><span className="flex shrink-0 items-center gap-2 font-mono text-[11px] text-slate-500 dark:text-slate-400"><b className="font-sans text-[10px] font-medium">{percentage}%</b>{hidden ? '••••' : money(value)}</span></div><div className="h-1.5 overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800"><div className={`h-full rounded-full ${colors[index]}`} style={{width:`${percentage}%`}}/></div></div>})}</div> : <p className="rounded-xl bg-slate-50 p-3 text-xs text-slate-500 dark:bg-slate-950 dark:text-slate-400">Todavía no hay gastos para mostrar este mes.</p>}</div>
