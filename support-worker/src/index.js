...
export default {
  async fetch(request, env) {
    ...
    if(url.pathname==='/iol/sync'&&request.method==='POST'){
      ...
      const targetEmail=String(env.IOL_TARGET_EMAIL||'').trim().toLowerCase(),valuationArs=Number(payload.valuationArs),initialInvestedArs=Number.isFinite(Number(payload.initialInvestedArs))?Number(payload.initialInvestedArs):108893,totalGainArs=Number.isFinite(Number(payload.totalGainArs))?Number(payload.totalGainArs):valuationArs-initialInvestedArs,totalGainPct=Number.isFinite(Number(payload.totalGainPct))?Number(payload.totalGainPct):(initialInvestedArs>0?totalGainArs/initialInvestedArs*100:0),dailyChangeArs=Number(payload.dailyChangeArs||0),dailyChangePct=Number(payload.dailyChangePct||0),marketDate=String(payload.marketDate||'');
